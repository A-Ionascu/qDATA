...
packages <- c("ggplot2", "dplyr","tidyr","shiny","shinythemes","DT",
              ...
              "fontawesome","RColorBrewer","ggtext","coin","exactRankTests")
...
if(interactive()){
  ...
  ui <- shinyUI( navbarPage( 
    ...
    title="qDATA",
    main_page <-  tabPanel(title=HTML(paste0("Livak 2", tags$sup("-\u0394\u0394Ct") ," method")), value="livak",
                           titlePanel(HTML(paste0("Livak 2", tags$sup("-\u0394\u0394Ct")," method"))),
                           sidebarLayout(
                             sidebarPanel(#style = "position:fixed;width:20%;",
                                          title="File input",
                                          fileInput("file", "Choose .csv file",
                                                    accept = c('.csv'), buttonLabel=list(icon("upload"), paste("Browse"))),
                                          width = 3,
                                          br(),
                             ...
                             mainPanel(
                               tabsetPanel(
                                 ...
                                 tabPanel(title="Two samples testing", value="pairs",
                                          ...
                                          selectInput("select_paired","Select a statistical test:", 
                                 ...
                                 tabPanel(title="Multiple samples testing", value="multiple",
                                          ...
                                          br(),br(),
                                          h4("Select the type of statistical test:"),
                                          selectInput("select_multiple","Select a statistical test:", 
    ...
    about_page <- tabPanel(title="About",
                           ...
                           h5("March-July 2023"),
                           br(),
                           h3("For citing our tool, please use: TBP"),
                           ...
                           br(),
                           h4(HTML(paste0("2", tags$sup("-\u0394Ct")," summary tab:"))),
  ...
  server <- function(input, output, session){ 
    ...
    outputDct <- reactive({
      ...
      outputDct_table <- na.omit(outputDct_table)
      outputDct_table
    })
    ...
    output$output2Dctvalues <- downloadHandler(
      filename = function(){paste("2Dct_table_",gsub(":","-",format(Sys.time(),'%d-%m-%Y_%H-%M-%S')), ".csv", sep="")},
    ...
    summary2Dct <- reactive({
      ...
      for(i in seq(1, ncol(output2Dct_table), 1)){
        ...
        summary2Dct$Mean[i] <- decimals(mean(c(output2Dct_table[,i])),decVal())
        summary2Dct$Median[i] <- decimals(median(c(output2Dct_table[,i])),decVal())
        summary2Dct$SD[i] <- decimals(sd(c(output2Dct_table[,i])),decVal())
        summary2Dct$Variance[i] <- decimals(var(c(output2Dct_table[,i])),decVal())
        summary2Dct$SE[i] <- decimals(SE(c(output2Dct_table[,i])),decVal())
        summary2Dct$`CI-lower`[i] <- decimals(CI_lower(c(output2Dct_table[,i])),decVal())
        summary2Dct$`CI-upper`[i] <- decimals(CI_upper(c(output2Dct_table[,i])),decVal())
        summary2Dct$IQR[i] <- decimals(IQR(c(output2Dct_table[,i])),decVal())
        summary2Dct$Minimum[i] <- decimals(min(c(output2Dct_table[,i])),decVal())
        summary2Dct$Maximum[i] <- decimals(max(c(output2Dct_table[,i])),decVal())
      }
      ...
      boxplot <- reactive({
        boxplot_2Dct <- stack(output2Dct_table) %>%
      ...
      violin <- reactive({
        violin_2Dct <- stack(output2Dct_table) %>%
          ggplot( aes(x=ind, y=values, fill=ind)) +
      ...
      histogram <- reactive({
        histogram_2Dct <- stack(output2Dct_table)
    ...
    normality <- reactive({
      ...
      normality_table <- data.frame(matrix(NA, nrow=ncol(output2Dct_table)*4, ncol=5))
      ...
      for(i in seq(1, ncol(output2Dct_table),1)){
        ...
        if(ks$p.value <= 0.05){ normality_table[rand[i],5] <- paste("Parametric") }
        ...
        if(sw$p.value > 0.05){ normality_table[rand[i]+1,5] <- paste("Parametric") }
        ...
        if(sw$p.value > 0.05){  normality_table[rand[i]+2,5] <- paste("Parametric") }
      ...
      qqplot <- reactive({
        ...
        qqplot_table <- stack(output2Dct_table)
        j<-1
      ...
      if(input$typeofdata == "2^_transformed"){
          ...
          output$paired <- DT::renderDataTable(
            DT::datatable(paired_tests(),filter = "top",
                          options = list(pageLength=-1,lengthMenu=list(c(10,50,100,-1),c('10','50','100','All')),
                                         columnDefs = list(list(className = 'dt-center', targets = 2:8))),escape=FALSE ))
        ...
        if(input$select_paired == "wilcoxon"){
          ...
          output$paired <- DT::renderDataTable(
            DT::datatable(paired_tests(),filter = "top",
                          options = list(pageLength=-1,lengthMenu=list(c(10,50,100,-1),c('10','50','100','All')),
                                         columnDefs = list(list(className = 'dt-center', targets = 2:7))),escape=FALSE ))
          download_paired_tests <- reactive({
            download_paired_tests <- paired_tests()
            for(i in seq(1, nrow(download_paired_tests),1)){
              if(download_paired_tests$p[i] <= pval()){
    ...
    ANOVA <- reactive({
      ...
      pre_anova <- stack(output2Dct_table)
      anova_table  <- data.frame(matrix(NA, nrow = 1, ncol=6))
      colnames(anova_table) <- c("Comparison","Significance","F","p", "DFg", "DFv")
      ...
      anova_table$Comparison[1] <- paste("ANOVA between chosen groups")
    ...
    WELCHANOVA <- reactive({
      ...
      anova_table$Comparison[1] <- paste("Welch's ANOVA between chosen groups")
    ...
    KRUSKALWALLIS <- reactive({
      ...
      pre_KW <- stack(output2Dct_table)
      KW_table  <- data.frame(matrix(NA, nrow = 1, ncol=5))
      colnames(KW_table) <- c("Comparison","Significance","Statistic","p", "df")
      
      KW <- kruskal.test(values~ind, data=pre_KW)
      
      KW_table$Comparison[1] <- KW$method
    ...
    DUNN <- reactive({
      ...
      pre_kruskal <- stack(output2Dct_table)
      ...
      if(input$typeofdata == "2^_transformed"){
          ...
          output$multiple_table <- DT::renderDataTable(
            DT::datatable(multiple_test(),
                          options = list(lengthChange = FALSE, searching=FALSE, paging=FALSE,
                                         columnDefs = list(list(className = 'dt-center', targets = 2:6))),escape=FALSE ))
        ...
        if(input$select_multiple == "kruskalwallis"){
          ...
          output$multiple_table <- DT::renderDataTable(
            DT::datatable(multiple_test(),
                          options = list(lengthChange = FALSE, searching=FALSE, paging=FALSE,
                                         columnDefs = list(list(className = 'dt-center', targets = 2:5))),escape=FALSE ))
          download_multiple_test <- reactive({
            download_multiple_tests <- multiple_test()
            for(i in seq(1, nrow(download_multiple_tests),1)){
              if(download_multiple_tests$p[i] <= pval()){
        ...
        for(j in seq(1,ncol(outputDct_table),1)){
          if(i != j){
            ...
            if(any(sqsum == iteration)){ next }
              ...
              sqsum <- append(sqsum, iteration)
              Livak(x=i, y=j)
              colnames(DDct_table)[s] <- paste(colnames(outputDct_table)[i]," vs ",colnames(outputDct_table)[j])
              livak <- 2^(-DDct)
              if (length(livak) == length(DDct_table[,s])) {
                DDct_table[,s] <- c(livak) }
              else {
                DDct_table[,s] <- c(livak, rep(NA, length(DDct_table[,s]) - length(livak))) }
              s <- s+1
              DDct <- c()
            }
          }}}
    ...
    output$fc <- DT::renderDataTable(
      DT::datatable(foldchange(), filter = "top",
                    options = list( pageLength = -1,
                                    lengthMenu = list(c(10,50,100,-1),c('10','50','100','All')),
                                    columnDefs = list(list(className = 'dt-center', targets = 2:12))), escape = FALSE))
